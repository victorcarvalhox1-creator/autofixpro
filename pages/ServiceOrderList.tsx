    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-gray-500">Gerencie todas as entradas e saídas da oficina.</p>
        </div>
        <button 
            onClick={() => navigate('/orders/new')}
            className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 shadow-lg shadow-blue-600/20 transition-all"
        >
            <Plus size={20} />
            Nova OS
        </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex gap-4 bg-gray-50">
            ...
            <button className="flex items-center gap-2 px-4 py-2 border border-gray-300 bg-white rounded-lg text-gray-700 hover:bg-gray-50 text-sm font-medium">
                <Filter size={18} />
                Filtros
            </button>
        </div>
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={os.id} className="hover:bg-blue-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-500">{new Date(os.entryDate).toLocaleDateString('pt-BR')}</td>
                    <td className="px-6 py-4 text-gray-500">{new Date(os.deliveryForecast).toLocaleDateString('pt-BR')}</td>
                    <td className="px-6 py-4 text-right font-medium text-gray-900">
                        R$ {os.finalPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-right">
                        <Link to={`/orders/${os.id}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm opacity-0 group-hover:opacity-100 transition-opacity">
                            Detalhes
                        </Link>
                    </td>
                </tr>
